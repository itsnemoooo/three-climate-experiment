...
class DNN_3(nn.Module):
    ...
    def reinitialize_last_layers(self):
        torch.nn.init.kaiming_uniform_(self.layer5[0].weight, nonlinearity='relu')
        torch.nn.init.kaiming_uniform_(self.layer6[0].weight, nonlinearity='relu')

class ReplayBuffer:
    def __init__(self, capacity):
        timestamp = datetime.datetime.now().strftime('%Y%m%d_%H%M%S')
        self.save_path = f'./final_results/prior{timestamp}.csv'
...
class DQN:
    def __init__(self, state_dim, action_dim, learning_rate, gamma,
                 epsilon, epsilon_min, epsilon_decay, target_update, device, reinitialize_frequency):
        ...
        self.reinitialize_frequency = reinitialize_frequency
    ...
    def update(self, replay_buffer, batch_size):
        ...
        if self.count % self.reinitialize_frequency == 0:
            self.q_net.reinitialize_last_layers()
        self.count += 1
...
def callback_function_DQN(state_argument, api, EPLUS):
    ...
    if time_interval % EPLUS.FPS == 0:
        ...
        #if EPLUS.RL_flag == True & time_interval > 0:
            #print(f'{time_interval} {T_31} | {70} {reward_T} | {reward_E} | {reward_signal} {action_1} | {action_map}')
            #print('%d / _%s %.2f / 72 %.3f / %.3f / %.2f (T/E/S)'%(time_interval,dt,T_mean[-1],reward_T,reward_E,reward_signal))
...
if __name__ == '__main__':
    ...
    timestamp = datetime.datetime.now().strftime('%Y%m%d_%H%M%S')
    all_results_df.to_csv(f'Random_weight_resetting_K{timestamp}.csv', index=False)

    print(f"Results saved to Random_weight...{timestamp}.csv")
